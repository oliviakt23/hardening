...
[5~touch /etc/at.allow /etc/cron.allow
chown root:root /etc/at.allow /etc/cron.allow
[5~chmod 600 /etc/at.allow /etc/cron.allow
...
[5~-a always,exit -F arch=b64 -S sethostname -S setdomainname -k system-locale
-a always,exit -F arch=b32 -S sethostname -S setdomainname -k system-locale
[5~[5~[5~-w /etc/issue -p wa -k system-locale
-w /etc/issue.net -p wa -k system-locale
[5~-w /etc/hosts -p wa -k system-locale
[5~-w /etc/sysconfig/network -p wa -k system-locale
...
[5~[5~
[5~-w /var/run/utmp -p wa -k session
-w /var/log/wtmp -p wa -k session
-w /var/log/btmp -p wa -k session
[5~
[5~[5~-a always,exit -F arch=b64 -S chmod -S fchmod -S fchmodat -F auid>=1000 -F auid!=4294967295 -k perm_mod
...
[5~[5~[5~-a always,exit -F arch=b64 -S unlink -S unlinkat -S rename -S renameat -F auid>=1000 -F auid!=4294967295 -k delete
-a always,exit -F arch=b32 -S unlink -S unlinkat -S rename -S renameat -F auid>=1000 -F auid!=4294967295 -k delete
[5~
-w /etc/sudoers -p wa -k scope
[5~[5~-w /etc/sudoers.d -p wa -k scope

-w /var/log/sudo.log -p wa -k actions

-w /sbin/insmod -p x -k modules
[5~-w /sbin/rmmod -p x -k modules
[5~-w /sbin/modprobe -p x -k modules
[5~-a always,exit -F arch=b64 -S init_module -S delete_module -k modules
[5~[5~[5~
...
[5~[5~sed -i "${line_num} a MACs hmac-sha1-etm@openssh.com,umac-64-etm@openssh.com,umac-128-etm@openssh.com,hmac-sha2-256-etm@openssh.com,hmac-sha2-512-etm@openssh.com,hmac-ripemd160-etm@openssh.com,hmac-sha1,umac-64@openssh.com,umac-128@openssh.com,hmac-sha2-256,hmac-sha2-512,hmac-ripemd160" ${sshd_config}  # CIS 5.2.12
sed -i "${line_num} a Ciphers aes128-ctr,aes192-ctr,aes256-ctr" ${sshd_config}  # CIS 5.2.11

[5~sed -i "s/\#ClientAliveInterval 0/ClientAliveInterval 300/" ${sshd_config}	# CIS 5.2.13
[5~[5~sed -i "s/\#ClientAliveCountMax 3/ClientAliveCountMax 0/" ${sshd_config}	# CIS 5.2.13
...
[5~[5~auth [success=1 default=bad] pam_unix.so
auth [default=die] pam_faillock.so authfail audit deny=5 unlock_time=900
auth sufficient pam_faillock.so authsucc audit deny=5 unlock_time=900
auth required pam_deny.so\n$content" > /etc/pam.d/password-auth

content="$(egrep -v "^#|^auth" /etc/pam.d/system-auth)"
[5~[5~[5~echo -e "auth required pam_env.so
[5~auth sufficient pam_unix.so remember=5
[5~[5~auth required pam_faillock.so preauth audit silent deny=5 unlock_time=900
...
[5~
[5~[5~[5~[5~login_defs=/etc/login.defs
sed -i 's/^PASS_MAX_DAYS.*$/PASS_MAX_DAYS 90/' ${login_defs}		# CIS 5.4.1.1
[5~sed -i 's/^PASS_MIN_DAYS.*$/PASS_MIN_DAYS 7/' ${login_defs}		# CIS 5.4.1.2
